      ...
      <main className="px-4 pb-24 pt-32 sm:px-6 sm:pt-36 lg:px-8">
        <section className="mx-auto max-w-7xl">
                ...
                <div className="relative">
                  <div className="flex flex-wrap items-center gap-3">
                    <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-4 py-2 text-xs font-black uppercase tracking-[0.15em] text-cyan-200">
                      <MapPin className="h-4 w-4" /> {hub.district}
                    </span>
                    <span
                      className={
                  ...
                  <h1 className="mt-7 text-5xl font-black tracking-[-0.045em] sm:text-6xl">
                    Hanova {hub.city} Hub
                  </h1>
                  ...
                  <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                    <a
                      href={"/events/?hub=" + hub.slug}
                      className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-white px-7 text-sm font-black text-slate-950"
                    >
                      <CalendarDays className="h-4 w-4" /> View hub events
                    </a>
                    <a
                      href="/community/join"
                      className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-white/20 bg-white/10 px-7 text-sm font-black text-white"
                    >
                      Join this community <ArrowRight className="h-4 w-4" />
                    </a>
                  </div>
                </div>
              </div>
              <div className="border-t border-white/10 bg-gradient-to-br from-blue-600/20 to-violet-500/10 p-7 sm:p-10 lg:border-l lg:border-t-0 lg:p-12">
                ...
                <p className="mt-3 text-2xl font-black">
                  {events.length > 0
                    ...
                    : "Community interest is open"}
                </p>
                <p className="mt-4 leading-7 text-slate-300">
                  Hub programming grows around the needs of local founders and the strength of
                  regional partners.
                </p>
              </div>
            </div>
          ...
          <div className="mt-10 grid gap-5 lg:grid-cols-2">
            <HubList
              title="What founders can explore"
              label="Focus areas"
              items={hub.focusAreas}
              color="blue"
            />
            <HubList
              title="What the network provides"
              label="Hub access"
              items={hub.facilities}
              color="violet"
            />
          </div>
        </section>
      ...
      <div className="mt-6 space-y-4">
        {items.map((item) => (
          <div key={item} className="flex items-center gap-3 font-bold text-slate-700">
            <CheckCircle2
              className={"h-5 w-5 " + (color === "blue" ? "text-blue-600" : "text-violet-600")}
            />{" "}
            {item}
          </div>
        ))}
