          ...
          {filteredChapters.length > 0 ? (
            <>
              <div className="mb-4 mt-7 flex items-center justify-between sm:hidden">
                <p className="text-sm font-semibold text-slate-600">Swipe through chapters</p>
              ...
              <div className="-mx-4 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:mx-0 sm:mt-8 sm:grid sm:grid-cols-2 sm:gap-5 sm:overflow-visible sm:px-0 sm:pb-0 lg:grid-cols-3">
                {filteredChapters.map((chapter) => (
                  <article
                    key={chapter.slug}
                    className="group flex min-h-[340px] min-w-[84%] snap-start flex-col rounded-[26px] border border-slate-200 bg-white p-5 shadow-[0_20px_65px_-48px_rgba(15,23,42,0.6)] transition duration-300 hover:-translate-y-1.5 hover:border-blue-200 sm:min-w-0 sm:rounded-[28px] sm:p-6"
                  >
