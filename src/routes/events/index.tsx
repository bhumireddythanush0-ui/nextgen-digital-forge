            ...
            <div className="mt-10 grid gap-6 lg:grid-cols-2">
              {events.map((event) => {
                ...
                return (
                  <article
                    key={event.id}
                    className="group overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-[0_25px_70px_-50px_rgba(15,23,42,0.6)]"
                  >
                    ...
                    <div className="p-7 sm:p-9">
                      ...
                      <div className="mt-7 grid gap-3 text-sm font-bold text-slate-700 sm:grid-cols-2">
                        <p className="flex items-center gap-2">
                          <CalendarDays className="h-4 w-4 text-blue-600" /> {event.displayDate}
                        </p>
                        <p className="flex items-center gap-2">
                          <Clock3 className="h-4 w-4 text-blue-600" /> {event.time}
                        </p>
                        <p className="flex items-start gap-2 sm:col-span-2">
                          <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-blue-600" /> {event.venue}
                        </p>
                      </div>
                      <a
                        href={"/events/register?event=" + event.id}
                        className="mt-8 inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-slate-950 px-7 text-sm font-black text-white transition hover:bg-blue-600"
                      >
                        Register for meetup <ArrowRight className="h-4 w-4" />
                      </a>
                    </div>
                  </article>
                );
