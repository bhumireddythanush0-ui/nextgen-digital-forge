      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col justify-between gap-8 lg:flex-row lg:items-end">
          <div className="max-w-3xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-4 py-2 text-xs font-bold uppercase tracking-[0.16em] text-blue-700">
              ...
              <span>Founder ecosystem overview</span>
            </div>
          ...
          <div className="w-full rounded-[24px] bg-slate-950 p-5 text-white shadow-xl sm:w-auto sm:min-w-[250px] sm:rounded-[28px] sm:p-6">
            <p className="text-xs font-bold uppercase tracking-[0.16em] text-cyan-300">
              Total founders
            </p>
            <p className="mt-2 text-4xl font-black sm:text-5xl">{totalFounders}</p>
            <p className="mt-2 text-sm leading-6 text-slate-300">
              Represented across five startup stages
            </p>
          </div>
        </div>
        ...
        <div className="mt-10 sm:mt-12">
          <div className="mb-4 flex items-center justify-between sm:hidden">
            <p className="text-sm font-semibold text-slate-600">Swipe to explore</p>
            <p className="text-xs font-bold uppercase tracking-wider text-blue-600">5 stages →</p>
          </div>

          <div className="-mx-4 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:mx-0 sm:grid sm:grid-cols-2 sm:gap-5 sm:overflow-visible sm:px-0 sm:pb-0 lg:grid-cols-5">
            {startupStages.map((stage, index) => {
              ...
              return (
                <article
                  key={stage.stage}
                  className="group min-w-[84%] snap-start rounded-[24px] border border-slate-200 bg-slate-50 p-5 transition duration-300 hover:-translate-y-1 hover:border-blue-300 hover:bg-white hover:shadow-xl sm:min-w-0 sm:rounded-[28px] sm:p-6"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-slate-950 text-white transition group-hover:bg-blue-600 sm:h-12 sm:w-12">
                  ...
                  <p className="mt-6 text-xs font-bold uppercase tracking-[0.16em] text-slate-500 sm:mt-7">
        ...
        <div className="mt-7 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-4 text-center text-sm leading-6 text-amber-900 sm:mt-8 sm:px-5">
