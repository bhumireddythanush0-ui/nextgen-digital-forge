...
export function AboutFounders() {
  return (
    <section
      id="founders"
      className="relative scroll-mt-28 overflow-hidden bg-white py-16 sm:py-24 lg:py-28"
    >
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-4 mt-12 flex items-center justify-between sm:hidden">
          <p className="text-sm font-semibold text-slate-600">Swipe to meet the founders</p>
          <p className="text-xs font-bold uppercase tracking-wider text-blue-600">2 founders →</p>
        </div>

        <div className="-mx-4 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:mx-0 sm:grid sm:grid-cols-2 sm:gap-6 sm:overflow-visible sm:px-0 sm:pb-0 lg:mt-14">
          {founders.map((founder, index) => (
            <article
              key={founder.name}
              className="group min-w-[88%] snap-start overflow-hidden rounded-[26px] border border-slate-200 bg-white shadow-[0_30px_80px_-45px_rgba(15,23,42,0.45)] transition duration-300 hover:-translate-y-1 hover:border-blue-300 sm:min-w-0 sm:rounded-[30px]"
            >
              <div className="relative h-[340px] overflow-hidden bg-slate-200 sm:h-[430px]">
                <img
                  src={founder.image}
              ...
              <div className="p-6 sm:p-8">
                ...
                <a
                  href={`/founders/connect?founder=${encodeURIComponent(founder.name.toLowerCase())}`}
                  className="mt-7 inline-flex min-h-11 items-center gap-2 text-sm font-bold text-slate-950 transition hover:text-blue-600"
                >
                  Book Meeting
